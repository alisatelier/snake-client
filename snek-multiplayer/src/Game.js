...
const { broadcastLogCrash, broadcastLogHitSnake } = require("../../constants.js");
/**
 * @class Game
 *
  ...
  checkPlayerHits() {
    ...
    for (let [i, snake] of this.snakes.entries()) {
      if (snake.hit(width, height)) {
        return this.removeSnake(snake, i, broadcastLogCrash);
      ...
      if (SNAKE_COLLISIONS) {
        for (let [j, otherSnake] of this.snakes.entries()) {
          if (i !== j && snake.hitSnake(otherSnake)) {
            return this.removeSnake(snake, i, broadcastLogHitSnake);
          }
        }
      }
    }
